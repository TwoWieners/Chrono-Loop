...
namespace Epoch
{
	...
	struct CCEnterLevel5 : public CodeComponent
	{
		...
		virtual void Update()
		{
			if (!once)
			{
				...
				next->BinaryLoadLevel(LevelManager::GetInstance().LevelOrder[5]);
				...
				if (/*status == LM::LevelStatus::Success*/ true)
				{
					...
					((AudioEmitter*)ambient)->AddEvent(Emitter::EventType::ePlay, AK::EVENTS::PLAY_LEVEL5AMBIENT);
					((AudioEmitter*)ambient)->AddEvent(Emitter::EventType::ePause, AK::EVENTS::PAUSE_LEVEL5AMBIENT);
					((AudioEmitter*)ambient)->AddEvent(Emitter::EventType::eResume, AK::EVENTS::RESUME_LEVEL5AMBIENT);
					((AudioEmitter*)ambient)->AddEvent(Emitter::EventType::eStop, AK::EVENTS::STOP_LEVEL5AMBIENT);
					...
					ParticleEmitter* startEmit = new TeleportEffect(400, 250, 2, vec4f(-5.7f, 0, 4.95f, 1));
					...
					ParticleEmitter* startEmit2 = new TeleportEffect(400, 150, 1, vec4f(-5.7f, 0, 4.95f, 1));
