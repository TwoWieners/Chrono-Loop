...
namespace Epoch
{
	...
	struct CCEnterLevel2 : public CodeComponent
	{
		...
		virtual void Update()
		{
			if (!once)
			{
				...
				next->BinaryLoadLevel(LevelManager::GetInstance().LevelOrder[2]);
				...
				if (/*status == LM::LevelStatus::Success*/ true)
				{
					...
					ParticleEmitter* startEmit = new TeleportEffect(400, 250, 2, vec4f(6, 0, 2, 1));
					...
					ParticleEmitter* startEmit2 = new TeleportEffect(400, 150, 1, vec4f(6, 0, 2, 1));
					...
					p2->SetLife(500);
					p2->SetSize(.35f, .15f);
					ParticleEmitter* emit21 = new TeleportEffect(-1, 150, 2, vec4f(-5.00f, 0, -4.71f, 1));
					...
					p2->SetLife(500);
					p2->SetSize(.15f, .05f);
					ParticleEmitter* emit22 = new TeleportEffect(-1, 150, 2, vec4f(-5.00f, 0, -4.71f, 1));
