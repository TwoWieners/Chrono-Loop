...
namespace Epoch {
	...
	struct CCEnterLevel4 : public CodeComponent {
		...
		virtual void Update() {
			if (!once) {
				...
				next->BinaryLoadLevel(LevelManager::GetInstance().LevelOrder[4]);
				...
				if (/*status == LM::LevelStatus::Success*/ true) {
					...
					p2->SetLife(500);
					p2->SetSize(.35f, .15f);
					ParticleEmitter* emit21 = new TeleportEffect(-1, 150, 2, vec4f(-13.39f, -3.48f, 2.44f, 1));
					...
					p2->SetLife(500);
					p2->SetSize(.15f, .05f);
					ParticleEmitter* emit22 = new TeleportEffect(-1, 150, 2, vec4f(-13.39f, -3.48f, 2.44f, 1));
