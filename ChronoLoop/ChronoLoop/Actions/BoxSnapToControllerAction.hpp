	...
	struct BoxSnapToControllerAction : public CodeComponent {
		...
		virtual void Update() override {
			...
			if (VRInputManager::Instance().IsInitialized()) {
				Controller &leftController = VRInputManager::Instance().GetController(true), &rightController = VRInputManager::Instance().GetController(false);
...
#pragma endregion Gestures
		...
		virtual void SnapToController(bool left) {
			mHeld = true;
			mHeldLeft = left;
			matrix4 m = VRInputManager::Instance().GetController(left).GetPosition();
		...
		virtual void ReleaseCube() {
			vec4f force = VRInputManager::Instance().GetController(mHeldLeft).GetVelocity();
